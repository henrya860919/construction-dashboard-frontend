      ...
      <nav class="relative flex flex-col gap-2 p-2" :class="collapsed ? 'items-center' : ''">
              ...
              >
                <RouterLink v-slot="{ navigate }" :to="item.path" custom>
                  <Tooltip v-if="collapsed">
                    <TooltipTrigger as-child>
                      <Button
                        variant="ghost"
                        size="icon"
                        :class="cn('h-9 w-9 shrink-0 justify-center rounded-md', isPlatformItemActive(item.path) && 'bg-accent text-accent-foreground')"
                        @click="navigate"
                      >
                  ...
                  <Button
                    v-else
                    variant="ghost"
                    :class="cn('h-9 w-full justify-start gap-3 rounded-md px-3', isPlatformItemActive(item.path) && 'bg-accent text-accent-foreground')"
                    @click="navigate"
                  >
                    <component :is="ICON_MAP[item.icon] ?? LayoutDashboard" class="size-4 shrink-0" />
        ...
        <template v-else-if="isAdminScope">
          ...
          >
            <Tooltip v-if="collapsed">
              <TooltipTrigger as-child>
                <Button
                  variant="ghost"
                  size="icon"
                  :class="cn('h-9 w-9 shrink-0 justify-center rounded-md', isItemActive(item.path) && 'bg-accent text-accent-foreground')"
            ...
            <Button
              v-else
              variant="ghost"
              :class="cn('h-9 w-full justify-start gap-3 rounded-md px-3', isItemActive(item.path) && 'bg-accent text-accent-foreground')"
              @click="router.push(item.path)"
            >
              <component :is="ICON_MAP[item.icon] ?? LayoutDashboard" class="size-4 shrink-0" />
              <span class="truncate">{{ item.label }}</span>
        ...
        <template v-else-if="isProjectScope">
          <div class="relative min-h-[200px] w-full overflow-hidden">
            ...
            <div
              class="flex w-[200%]"
              :style="{ transform: sidebarStore.currentPanel === 'root' ? 'translateX(0)' : 'translateX(-50%)', transition: 'transform 260ms cubic-bezier(0.4, 0, 0.2, 1)' }"
            >
              ...
              <div class="w-1/2 shrink-0 px-1">
                ...
                <template v-for="(item, idx) in LAYER2_ITEMS" :key="item.type === 'group' ? `group-${idx}` : item.id">
                  <!-- 群組標籤：不可點擊 -->
                  <div
                  ...
                  >
                    <Tooltip v-if="collapsed">
                      <TooltipTrigger as-child>
                        <Button
                          variant="ghost"
                          size="icon"
                          :class="cn('h-9 w-9 shrink-0 justify-center rounded-md', isProjectChildActive(item.pathSuffix) && 'bg-accent text-accent-foreground')"
                    ...
                    <Button
                      v-else
                      variant="ghost"
                      :class="cn('h-9 w-full justify-start gap-3 rounded-md px-3', isProjectChildActive(item.pathSuffix) && 'bg-accent text-accent-foreground')"
                      @click="goToProjectPath(item.pathSuffix)"
                    >
                      <component :is="ICON_MAP[item.icon] ?? LayoutDashboard" class="size-4 shrink-0" />
                      <span class="truncate">{{ item.label }}</span>
              ...
              <div v-if="layer3Items.length" class="w-1/2 shrink-0 px-1">
                ...
                >
                  <Tooltip v-if="collapsed">
                    <TooltipTrigger as-child>
                      <Button
                        variant="ghost"
                        size="icon"
                        :class="cn('h-9 w-9 shrink-0 justify-center rounded-md', isProjectChildActive(child.pathSuffix) && 'bg-accent text-accent-foreground')"
                  ...
                  <Button
                    v-else
                    variant="ghost"
                    :class="cn('h-9 w-full justify-start gap-3 rounded-md px-3', isProjectChildActive(child.pathSuffix) && 'bg-accent text-accent-foreground')"
                    @click="goToProjectPath(child.pathSuffix)"
                  >
                    <component :is="ICON_MAP[child.icon] ?? LayoutDashboard" class="size-4 shrink-0" />
