...
import { getRepairRecord } from '@/api/repair-requests'
import { usePhotoViewer } from '@/composables/usePhotoViewer'
import type { RepairExecutionRecordItem } from '@/types/repair-request'
...
const record = ref<RepairExecutionRecordItem | null>(null)

async function fetchRecord() {
  if (!projectId.value || !repairId.value || !recordId.value) return
  loading.value = true
  try {
    record.value = await getRepairRecord(projectId.value, repairId.value, recordId.value)
  } finally {
    loading.value = false
  }
}

watch([projectId, repairId, recordId], fetchRecord, { immediate: true })
...
<template>
  <div class="mobile-page flex flex-col px-4 pb-6 pt-4">
    ...
    <template v-else-if="record">
      <div class="mb-4">
        <h2 class="text-lg font-semibold text-foreground">報修紀錄</h2>
      ...
      <div class="mt-auto pt-4">
        <Button variant="outline" class="min-h-12 w-full touch-manipulation" @click="goBack">
          返回報修詳情
        </Button>
      </div>
    </template>
    ...
    >
      <p class="text-sm text-muted-foreground">找不到該筆報修紀錄</p>
